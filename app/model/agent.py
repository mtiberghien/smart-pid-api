...
class Agent:
    ...
    def set_settings(self):
        ...
        if 'max_action' in settings:
            self.min_action = settings['max_action']
    ...
    def learn(self, update_actor):
        ...
        with tf.GradientTape() as tape:
            ...
            critic_value_ = tf.squeeze(self.target_critic(tf.concat([new_states, target_actions],axis=1)), 1)
            critic_value = tf.squeeze(self.critic(tf.concat([states, actions], axis=1)), 1)
        ...
        if update_actor:
            with tf.GradientTape() as tape:
                ...
                actor_loss = -self.critic(tf.concat([states, new_policy_actions], axis=1))
